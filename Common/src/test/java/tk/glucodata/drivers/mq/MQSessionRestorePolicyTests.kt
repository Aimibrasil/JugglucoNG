...
class MQSessionRestorePolicyTests {
    ...
    private val address = "CF:D8:EB:DD:F9:69"

    @Test fun rejectsAprilSnapshotFromSeptemberTrace() {
        assertFalse(MQSessionRestorePolicy.canRestore(address, "CFD8EBDDF969", 1777312377000L, now))
    }

    @Test fun acceptsCurrentSessionForSameTransmitter() {
        assertTrue(MQSessionRestorePolicy.canRestore(address, "cfd8ebddf969", now - day, now))
    }

    @Test fun rejectsUnrelatedOrUnidentifiedTransmitter() {
        assertFalse(MQSessionRestorePolicy.canRestore(address, "CFD8EBDDF968", now - day, now))
